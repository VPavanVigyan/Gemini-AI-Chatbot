...
def init_db():
    ...
    for column, definition in {
        ...
        if column not in existing_assessment:
            conn.execute(
                f"ALTER TABLE assessments ADD COLUMN {column} {definition}"
            )
...
def dashboard():
    ...
    history = conn.execute(
        """
        SELECT created_at, total_carbon
        FROM assessments
        WHERE user_id=?
        ORDER BY id ASC
        LIMIT 12
    ...
    if latest:
        recommendations = conn.execute(
            ...
            WHERE assessment_id=?
            ORDER BY estimated_reduction DESC
            """,
            (latest["id"],),
        ).fetchall()
...
def analyze():
    ...
    for key, default in numeric_defaults.items():
        data[key] = safe_float(data.get(key), default)
    ...
    data["operating_condition"] = (
        str(data.get("operating_condition") or "Normal")
        .strip()
        or "Normal"
    )
    ...
    data["daylight"] = max(
        0,
        min(data["daylight"], 100)
    )

    data["air_quality"] = max(
        0,
        min(data["air_quality"], 100)
    )

    data["carbon_intensity"] = max(
        0,
    ...
    data["temperature"] = max(
        -50,
        min(data["temperature"], 60)
    )

    data["operating_hours"] = max(
        0,
        min(data["operating_hours"], 24)
    )
...
def what_if():
    ...
    # Make sure the values required by calculate_impact
    # are never None.
    data["electricity_kwh"] = safe_float(
        data.get("electricity_kwh")
    )

    data["water_liters"] = safe_float(
        data.get("water_liters")
    )

    data["fuel_liters"] = safe_float(
        data.get("fuel_liters")
    )

    data["waste_kg"] = safe_float(
        data.get("waste_kg")
    )

    data["solar_kwh"] = safe_float(
        data.get("solar_kwh")
    )

    data["energy_cost"] = safe_float(
        data.get("energy_cost")
    )

    data["occupancy"] = safe_int(
        data.get("occupancy")
    )

    data["carbon_intensity"] = safe_float(
    ...
    er = max(
        ...
        min(
            safe_float(
                data.get("electricity_reduction_pct")
            ),
            100,
        ),
    )
...
def verify(assessment_id):
    ...
    return jsonify({
        ...
        "actual": actual,
        "variance": round(
            actual - row["predicted_reduction"],
            2,
        ),
    })
